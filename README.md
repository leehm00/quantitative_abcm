...
`h32`、`h48`、`h64` 是本项目的模型代号，数字表示 GRU 隐藏层维度。RankIC（秩信息系数）衡量因子排序与未来收益排序的相关性；ICIR（信息系数信息比率）衡量 RankIC 的稳定性；Top 组是 alpha 排名最高的 5%股票。表中的 Top 指标是未来 11 日 Top 5%平均收益相对同日股票池的超额收益，再乘以 `252/11` 得到的线性年化近似值。
...
PDF 数据集 1 报告的 Top 5% 正式组合年化收益为 34.51%，采用中证全指基准、组合净值和换手控制。该值与表内本地模型的标签超额年化近似值使用不同收益算法，因此不计算直接差值。PDF 的排序指标与本地五折评价区间也不同，只作量级参考。

结论概括：

- h32 的 alpha 综合指标最接近 PDF 参考值。
- h48 在 ABCM 候选中的 Top 5% 11 日超额年化近似值最高。
- h64 的 RankIC 最高，该收益指标接近 h48，跨验证折波动更低。
- PDF 基准损失提高了 beta 稳定性，同时 alpha 排序和 Top 5% 11 日超额年化近似值较低。
...
- RankIC、ICIR、胜率、命中率、Top 5% 11 日超额收益、long-short 收益、beta 自相关和滚动 R²评价。
...
- Top 5% 11 日超额年化近似值按 Top 5%股票未来收益减去同日股票池平均收益，再乘以 `252/11` 计算。
- 相邻 11 日收益窗口存在重叠。
- 收益未扣除交易成本、冲击成本和成交限制。
- PDF 的正式组合年化收益及每日净值回测的复合年化绝对收益采用其他计算方法，不与该近似值计算差值。
